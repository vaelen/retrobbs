...
{
  CRC32 Utility Program

  Calculates CRC-32/CKSUM (POSIX) checksum for files.
  Outputs the result as a hexadecimal number.

  Usage:
    crc32 <filename>    - Calculate CRC32 of file
...
begin
  ...
  begin
    WriteLn(StdErr, 'Usage: crc32 <filename>');
    Halt(1);
  end;
  ...
  begin
    ...
    if bytesRead > 0 then
      CRC32Add(crc, totalLen, buffer, bytesRead);
  end;

  Close(f);

  { Finalize CRC with length }
  crc := CRC32End(crc, totalLen);
